...
add_action('admin_enqueue_scripts', 'register_hss_woo_plugin_styles' );
/**
 * Register style sheet.
 */
...
?>
        <div class="wrap">
		<?php
		if(isset($_GET['check_user_video_access']) && wp_verify_nonce($_GET['_wpnonce'], 'hss_check_user_video_access')) {
		

$options = get_option('hss_woo_options');
$userId = (int)$_GET['user'];
$response = wp_remote_post( "https://www.hoststreamsell.com/api/1/xml/purchased_videos?api_key=".$options['api_key']."&private_user_id=$userId&database_id=".$options['database_id'], array(
                                        'method' => 'GET',
                                        'timeout' => 15,
                                        'redirection' => 5,
                                        'httpversion' => '1.0',
...
$index = 1;
        echo "<div class='hss_purchase_row_header'><div class='hss_purchase_title_header'>Video Title</div><div class='hss_purchase_bytes_used_header'>Usage</div><div class='hss_purchase_expires_header'>Expiry Details</div></div>";
while($index <= $count)
{
        ...
        echo "<div class='hss_purchase_row'><div class='hss_purchase_title'>".$title."</div><div class='hss_purchase_bytes_used'>".$usage."</div>";
        if($expires!="0")
                echo "<div class='hss_purchase_expires'>".$expires_on."</div></div>";
        else
                echo "<div class='hss_purchase_expires_blank'>n/a</div></div>";
        $index+=1;
}
                ...
                <form method="post" action="options.php">
                        ...
                        <table class="form-table">
                                ...
                                <tr>
                                        <th scope="row">Video Player<BR><i>Note: JW Player 7 requires a license file but they have changed their PRO plan to be free - see www.jwplayer.com for more info </i></th>
                                        <td>
                                                ...
                                                if ($options['jwplayer_version']=="6"){
                                                        ...
                                                        ?><option value="videojs5">Videojs</option><?php
                                                }
						...
						elseif ($options['jwplayer_version']=="videojs5"){
                                                        ?><option value="videojs5" SELECTED>Videojs</option><?php
                                                        ?><option value="7Prem">JW 7 Premium</option><?php
                                                        ?><option value="7">JW 7 Free</option><?php
                                                        ?><option value="6">JW 6 Free</option><?php
                                                } ?>
				...
				<tr>
						...
						<div><input type="button" value="Update" id="myajax" /></div>
                                ...
                                <tr>
                                        <th scope="row">Log JW Player Events</th>
                                        <td>
	...
	if($post->post_type == 'product' && is_singular() && is_main_query()) {
			...
			if(get_post_meta($post->ID, 'is_streaming_video', true)) {
				...
				$hss_video_id = get_post_meta($post->ID, '_woo_video_id', true);
                                        $response = wp_remote_post( "https://www.hoststreamsell.com/api/1/xml/videos?api_key=".$options['api_key']."&video_id=$hss_video_id&private_user_id=$userId&database_id=".$options['database_id']."&expands=playback_details&force_allow=no&limit=1000&offset=0&clientip=".get_the_user_ip(), array(
                                        'method' => 'GET',
                                        'timeout' => 15,
                                        'redirection' => 5,
                                        'httpversion' => '1.0',
				...
				if (is_iphone) { html5Player();}
					...
					jwplayer('videoframe').setup({
					    playlist: [{
				        	...
				        	sources: [{
					            file: '$httpString://www.hoststreamsell.com/mod/secure_videos/private_media_playlist_v2.php?params=".$hss_video_id."!".urlencode($referrer.":playersessionid:".$playersessionid)."!".$hss_video_user_token."!',
					            type: 'rtmp'
					        },{
				        	    file: 'http://".$hss_video_mediaserver_ip.":1935/hss/smil:".$hss_video_smil."/playlist.m3u8".$hss_video_smil_token."&referer=".urlencode($referrer.":playersessionid:".$playersessionid)."'
					        }]$subtitle_text
					    }],$captions
					 rtmp: {
                                                bufferlength: 10,
                                ...
                                function newJWPlayer()
                                {
                                        jwplayer('videoframe').setup({
                                            playlist: [{
                                                image: '$hss_video_big_thumb_url',
                                                sources: [{
                                                	file: '$httpString://$hss_video_mediaserver_url/hss/smil:".$hss_video_smil."/manifest.mpd".$hss_video_smil_token."&referer=".urlencode($referrer.":playersessionid:".$playersessionid)."',
                                                	type: 'dash'
                                                },{
                                                    	file: '$httpString://$hss_video_mediaserver_url/hss/smil:".$hss_video_smil."/playlist.m3u8".$hss_video_smil_token."&referer=".urlencode($referrer.":playersessionid:".$playersessionid)."'
                                                }]$subtitle_text
                                            }],$captions
                                            dash: 'shaka',";
if( ($options['jwplayer_version']=="7" or $options['jwplayer_version']=="7Prem") and $options['jwplayer_logo_file']!=""){
        $video.="                       logo: {
                                file: '".$options['jwplayer_logo_file']."',";
                        if($options['jwplayer_logo_link']!=""){
        $video.="
...
<body>
  ...
  data-setup='{}'>
  <source src=\"$httpString://$hss_video_mediaserver_url/hss/smil:".$hss_video_smil."/playlist.m3u8".$hss_video_smil_token."&referer=".urlencode($referrer.":playersessionid:".$playersessionid)."\" type='application/x-mpegURL'>";
                               ...
                               if($subtitle_count>0){
                                    while($subtitle_index <= $subtitle_count)
                                    {
                                            ...
                                            $subtitle_file = (string)$xml->result[0]->subtitles->{'subtitle_file_'.$subtitle_index}[0];
                                            $subtitle_index += 1;
                                            if(endsWith($subtitle_index,"vtt"))
                                            {
$video .= "
<track kind=\"captions\" src=\"$httpString://www.hoststreamsell.com/mod/secure_videos/subtitles/".$subtitle_file."?rand=".randomString."\" srclang=\"en\" label=\"".$subtitle_label."\">";
                                            }
...
if($options['log_player_events']=="true"){
  ...
  if($options['jwplayer_version']=="7" or $options['jwplayer_version']=="7Prem"){				
...
jwplayer().on('ready',function(e) {
	jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=ready&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&setupTime='+e.setupTime+'&userid=".$userId."&file='+jwplayer().getPlaylistItem(0).file,
                        dataType: 'jsonp',
                });
});
jwplayer().on('setupError',function(e) {
	jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=setupError&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&message='+e.message+'&userid=".$userId."',
                        dataType: 'jsonp',
                });
});
jwplayer().on('error',function(e) {
	jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=error&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&message='+e.message,
                        dataType: 'jsonp',
                });
...
jwplayer().on('buffer',function(e) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=buffer&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&oldstate='+e.oldstate+'&newstate='+e.newstate+'&reason='+e.reason,
                        dataType: 'jsonp',
                });
});
jwplayer().on('play',function(e) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=play&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&oldstate='+e.oldstate,
                        dataType: 'jsonp',
                });
});
jwplayer().on('pause',function(e) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=pause&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&oldstate='+e.oldstate,
                        dataType: 'jsonp',
                });
});
jwplayer().on('seek',function(e) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=seek&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&position='+e.position+'&offset='+e.offset,
                        dataType: 'jsonp',
                });
});
jwplayer().on('idle',function(e) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=idle&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&oldstate='+e.oldstate,
                        dataType: 'jsonp',
                });
});
jwplayer().on('complete',function(e) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=complete&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."',
                        dataType: 'jsonp',
                });
});
jwplayer().on('firstFrame',function(e) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=firstFrame&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&loadTime='+e.loadTime,
                        dataType: 'jsonp',
                });
});
jwplayer().on('levelsChanged',function(e) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=levelsChanged&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&currentQuality='+e.currentQuality,
                        dataType: 'jsonp',
                });
});
jwplayer().on('fullscreen',function(e) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=fullscreen&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&fullscreen='+e.fullscreen,
                        dataType: 'jsonp',
                });
});
jwplayer().on('resize',function(e) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw".$options['jwplayer_version']."&videoid=".$hss_video_id."&event=resize&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&width='+e.width+'&height='+e.height,
                        dataType: 'jsonp',
                });
});
  </script>
...
jwplayer().onReady(function(event) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw6&videoid=".$hss_video_id."&event=ready&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&file='+jwplayer().getPlaylistItem(0).file,
                        dataType: 'jsonp',
                });
});
jwplayer().onSetupError(function(event) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw6&videoid=".$hss_video_id."&event=setupError&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&message='+event.message,
                        dataType: 'jsonp',
                });
});
jwplayer().onError(function(event) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw6&videoid=".$hss_video_id."&event=error&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&message='+event.message,
                        dataType: 'jsonp',
                });
});
jwplayer().onBuffer(function(event) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw6&videoid=".$hss_video_id."&event=buffer&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&oldstate='+event.oldstate,
                        dataType: 'jsonp',
                });
});
jwplayer().onPlay(function(event) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw6&videoid=".$hss_video_id."&event=play&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&oldstate='+event.oldstate,
                        dataType: 'jsonp',
                });
});
jwplayer().onPause(function(event) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw6&videoid=".$hss_video_id."&event=pause&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&oldstate='+event.oldstate,
                        dataType: 'jsonp',
                });
});
jwplayer().onSeek(function(event) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw6&videoid=".$hss_video_id."&event=seek&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&position='+event.position+'&offset='+event.offset,
                        dataType: 'jsonp',
                });
});
jwplayer().onIdle(function(event) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw6&videoid=".$hss_video_id."&event=idle&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&oldstate='+event.oldstate,
                        dataType: 'jsonp',
                });
});
jwplayer().onComplete(function(event) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw6&videoid=".$hss_video_id."&event=complete&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."',
                        dataType: 'jsonp',
                });
});
jwplayer().onFullscreen(function(event) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw6&videoid=".$hss_video_id."&event=fullscreen&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&fullscreen='+event.fullscreen,
                        dataType: 'jsonp',
                });
});
jwplayer().onResize(function(event) {
        jQuery.ajax({
                        url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=jw6&videoid=".$hss_video_id."&event=resize&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&width='+event.width+'&height='+event.height,
                        dataType: 'jsonp',
                });
});
  </script>
             ...
             var videoreferrer=encodeURI(window.location.href);
                ...
                var player = this;
                 ...
                 player.on('play', function() {
                        jQuery.ajax({
                                dataType: 'jsonp',
                                url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=videojs5&videoid=".$hss_video_id."&event=play&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."',
                        });
                  });
                 player.on('error', function(event) {
                        ...
                        jQuery.ajax({
                                dataType: 'jsonp',
                                url: 'http://eventsapi.hoststreamsell.com/v1/logevent/ping.gif?player=videojs5&videoid=".$hss_video_id."&event=error&referrer='+videoreferrer+'&playersessionid=".$playersessionid."&browser='+agent+'&userid=".$userId."&message='+error.message+'&code='+error.code,
                        });
                  });
				...
				_log("STATUS: ".$status);
					...
					_log("Video count=".$count);
						...
						if($option_index==1){
							...
							if( $variations ) {
								foreach($variations as $variation_post)
									//_log("variation post_title=".$variation_post->post_title);
									//if (strpos($variation_post->post_title,'Access Period') !== false){
										wp_delete_post( $variation_post->ID);
										//_log("variation post_title=".$variation_post->post_title. "DELETED");
									//}
							}
                                                        ...
                                                        if( $variations ) {
                                                                foreach($variations as $variation_post)
                                                                        _log("AFTER DELETES variation post_title=".$variation_post->post_title);
							}
							...
							foreach($prices as $option_id => $option_values)
								...
								$new_variation = array(
									...
									'post_type'    => 'product_variation',
                                                                ...
                                                                _log('ADDING variation : Variation # '.$option_id.' of Access Period');
